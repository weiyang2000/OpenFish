...
4. **具体引用要求**：
   ...
   - **多源对比**：展示不同新闻源的表述差异
   - **时间线整理**：按时间顺序整理事件发展脉络
   - **来源链接**：搜索结果会在正文前提供 `[来源标题](URL)` 或“来源：...”标签；凡涉及数据、事实、引述、趋势判断的句子，必须就近保留对应的 Markdown 链接或来源标签
...
适当地组织段落结构以便纳入报告中。
凡新增或保留的数据、事实、引述、趋势判断，都必须就近保留搜索结果中的 `[来源标题](URL)` 或“来源：...”标签；禁止裸 URL、HTML 链接和不完整 Markdown 链接。
请按照以下JSON模式定义格式化输出：
...
- 所有数据、事实、引述、趋势判断句都要就近带来源链接；没有真实 URL 时使用原文中的“来源：...”标签。
