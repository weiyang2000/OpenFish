...
Crawler functionality uses CloakBrowser, which prepares its browser binary on first run. No separate browser-driver install command is required.
...
database, crawler accounts, and submodules before running; missing dependencies
now fail the task with a concrete error instead of producing placeholder output.
