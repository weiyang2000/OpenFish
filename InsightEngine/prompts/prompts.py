...
4. **情绪字段使用**：
   - 搜索结果会直接携带爬虫表内的 sentiment_label / sentiment_score 字段
   - 当需要分析情绪倾向时，优先要求能覆盖评论和用户原创内容的查询
   - 不要请求独立情感分析工具，系统不会在 InsightEngine 阶段重新跑模型
5. **参数优化配置**：
...
6. **阐述选择理由**：说明为什么这样的查询能够获得最真实的民意反馈
...
你可以使用以下5种专业的本地舆情数据库查询工具来深度挖掘民意，查询结果会携带爬虫表内情绪字段：
