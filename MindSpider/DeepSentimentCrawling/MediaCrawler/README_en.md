...
- **Core Technology**: Based on [CloakBrowser](https://github.com/CloakHQ/CloakBrowser) stealth browser contexts for login and maintaining login state
...
- ✅ **Simplified browser runtime dependency**, easier to use
...
> CloakBrowser prepares its browser binary on first run, so no separate browser-driver install step is required.
