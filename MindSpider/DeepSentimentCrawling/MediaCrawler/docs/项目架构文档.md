...
- **反爬虫对策**：CloakBrowser 隐身浏览器、代理 IP 池、请求签名
...
│   ├── cloak_browser.py    # CloakBrowser启动
...
```mermaid
flowchart LR
    launch["CloakBrowser启动"] --> context["浏览器上下文"]
    context --> f1["用户数据持久化"]
    context --> f2["代理/语言/时区配置"]
    context --> f3["隐身指纹能力"]
```
...
| 浏览器启动 | `cloak_browser.py` | CloakBrowser 上下文启动、持久化配置 |
