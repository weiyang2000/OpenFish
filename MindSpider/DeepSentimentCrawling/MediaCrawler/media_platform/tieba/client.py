...
class BaiduTieBaClient(AbstractApiClient):
    ...
    def __init__(
        ...
        self.browser_page = browser_page  # browser page object
    ...
    async def pong(self, browser_context: Any = None) -> bool:
        """
    ...
    async def get_notes_by_keyword(
        ...
        """
        Search Tieba posts by keyword (uses browser to access page, avoiding API detection)
        Args:
            keyword: Keyword
            page: Page number
            page_size: Page size
        ...
        if not self.browser_page:
            utils.logger.error("[BaiduTieBaClient.get_notes_by_keyword] browser_page is None, cannot use browser mode")
            raise Exception("browser_page is required for browser-based search")
        ...
        try:
            # Use browser to access search page
            await self.browser_page.goto(full_url, wait_until="domcontentloaded")
    ...
    async def get_note_by_id(self, note_id: str) -> TiebaNote:
        """
        Get post details by post ID (uses browser to access page, avoiding API detection)
        Args:
            note_id: Post ID
        ...
        """
        if not self.browser_page:
            utils.logger.error("[BaiduTieBaClient.get_note_by_id] browser_page is None, cannot use browser mode")
            raise Exception("browser_page is required for browser-based note detail fetching")
        ...
        try:
            # Use browser to access post detail page
            await self.browser_page.goto(note_url, wait_until="domcontentloaded")
    ...
    async def get_note_all_comments(
        ...
        """
        Get all first-level comments for specified post (uses browser to access page, avoiding API detection)
        Args:
        ...
        """
        if not self.browser_page:
            utils.logger.error("[BaiduTieBaClient.get_note_all_comments] browser_page is None, cannot use browser mode")
            raise Exception("browser_page is required for browser-based comment fetching")
        ...
        while note_detail.total_replay_page >= current_page and len(result) < max_count:
            ...
            try:
                # Use browser to access comment page
                await self.browser_page.goto(comment_url, wait_until="domcontentloaded")
    ...
    async def get_comments_all_sub_comments(
        ...
        """
        Get all sub-comments for specified comments (uses browser to access page, avoiding API detection)
        Args:
        ...
        if not self.browser_page:
            utils.logger.error("[BaiduTieBaClient.get_comments_all_sub_comments] browser_page is None, cannot use browser mode")
            raise Exception("browser_page is required for browser-based sub-comment fetching")
        ...
        for parment_comment in comments:
            ...
            while max_sub_page_num >= current_page:
                ...
                try:
                    # Use browser to access sub-comment page
                    await self.browser_page.goto(sub_comment_url, wait_until="domcontentloaded")
    ...
    async def get_notes_by_tieba_name(self, tieba_name: str, page_num: int) -> List[TiebaNote]:
        """
        Get post list by Tieba name (uses browser to access page, avoiding API detection)
        Args:
        ...
        """
        if not self.browser_page:
            utils.logger.error("[BaiduTieBaClient.get_notes_by_tieba_name] browser_page is None, cannot use browser mode")
            raise Exception("browser_page is required for browser-based tieba note fetching")
        ...
        try:
            # Use browser to access Tieba page
            await self.browser_page.goto(tieba_url, wait_until="domcontentloaded")
    ...
    async def get_creator_info_by_url(self, creator_url: str) -> str:
        """
        Get creator information by creator URL (uses browser to access page, avoiding API detection)
        Args:
        ...
        """
        if not self.browser_page:
            utils.logger.error("[BaiduTieBaClient.get_creator_info_by_url] browser_page is None, cannot use browser mode")
            raise Exception("browser_page is required for browser-based creator info fetching")

        utils.logger.info(f"[BaiduTieBaClient.get_creator_info_by_url] Accessing creator homepage: {creator_url}")

        try:
            # Use browser to access creator homepage
            await self.browser_page.goto(creator_url, wait_until="domcontentloaded")
    ...
    async def get_notes_by_creator(self, user_name: str, page_number: int) -> Dict:
        """
        Get creator's posts by creator (uses browser to access page, avoiding API detection)
        Args:
        ...
        """
        if not self.browser_page:
            utils.logger.error("[BaiduTieBaClient.get_notes_by_creator] browser_page is None, cannot use browser mode")
            raise Exception("browser_page is required for browser-based creator notes fetching")
        ...
        try:
            # Use browser to access creator post list page
            await self.browser_page.goto(creator_url, wait_until="domcontentloaded")
            ...
            try:
                ...
                json_text = await self.browser_page.evaluate("() => document.body.innerText")
