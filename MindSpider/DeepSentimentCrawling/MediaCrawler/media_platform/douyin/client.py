...
class DouYinClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def __process_req_params(
        ...
        local_storage: Dict = await self.browser_page.evaluate("() => window.localStorage")  # type: ignore
        common_params = {
        ...
        if "/v1/web/general/search" not in uri:
            a_bogus = await get_a_bogus(uri, query_string, post_data, headers["User-Agent"], self.browser_page)
    ...
    async def pong(self, browser_context: Any) -> bool:
        local_storage = await self.browser_page.evaluate("() => window.localStorage")
